...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from('pets')
    .select('id,name,species,age_years,created_at')
    .order('created_at', { ascending: false })

  if (error) return NextResponse.json({ error: 'Pets could not be loaded.' }, { status: 500 })
...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase
    .from('pets')
    .insert({ owner_id: user.id, name, species: petSpecies, age_years: age })
    .select('id,name,species,age_years,created_at')
    .single()
