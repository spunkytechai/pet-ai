...
export default function HistoryPage() {
  ...
  useEffect(() => {
    fetch('/api/history').then(async (response) => { const data = await response.json(); if (!response.ok) throw new Error(data?.error || 'History could not be loaded.'); setHistory(data.history ?? []) }).catch((err) => setError(err instanceof Error ? err.message : 'History could not be loaded.')).finally(() => setLoading(false))
  }, [])

  return (
    <main className="app-page">
      <div className="container">
        <header className="nav app-nav"><Link href="/" className="brand brand-lockup"><span className="brand-mark" aria-hidden="true">◒</span><span>PET AI</span></Link><nav className="nav-links"><Link className="secondary small" href="/analyze">Analyze a sound</Link><Link className="secondary small" href="/pet/new">+ Add pet</Link></nav></header>
        ...
        {error && <div className="card"><p className="safety">{error}</p></div>}
