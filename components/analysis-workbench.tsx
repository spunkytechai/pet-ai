...
export function AnalysisWorkbench() {
  ...
  return <div className="workbench">
    <div className="analysis-progress" aria-label="Analysis progress"><span className={stage >= 1 ? 'done' : ''}>01 <b>Capture</b></span><i className={stage >= 2 ? 'done' : ''}/><span className={stage >= 2 ? 'done' : ''}>02 <b>Context & analyze</b></span><i className={stage >= 3 ? 'done' : ''}/><span className={stage >= 3 ? 'done' : ''}>03 <b>Interpretation</b></span></div>
    <div className="card analysis-context-card">
      ...
      <label>{t.contextLabel}<textarea value={context} onChange={(e) => setContext(e.target.value)} placeholder="e.g. I had just returned home and my dog was at the door." rows={3} /><div className="context-hints"><button type="button" onClick={() => setContext('My pet was near food or asking for access.')} >Food / access</button><button type="button" onClick={() => setContext('I had just returned home and my pet came to the door.')} >Greeting</button><button type="button" onClick={() => setContext('Something changed in the environment and my pet noticed it.')} >Alert / environment</button><button type="button" onClick={() => setContext('My pet was alone or seemed unsettled.')} >Alone / unsettled</button></div></label>
    </div>
    ...
    <div className="card analysis-capture-card"><div className="eyebrow">01 · Capture</div><div className="capture-title-row"><div><h2>Record or upload a pet signal</h2><p className="muted">A short, clear vocalization gives the model a better acoustic signal.</p></div><span className="capture-limit">≤ 30 sec recommended</span></div><div className="actions"><AudioRecorder onAudioReady={acceptAudio} disabled={busy || !petId} /><button className="secondary" type="button" onClick={() => inputRef.current?.click()} disabled={busy || !petId}>{t.upload}</button><input ref={inputRef} hidden type="file" accept="audio/*" onChange={(e) => e.target.files?.[0] && void acceptAudio(e.target.files[0])} /></div>{file && <div className="audio-preview"><strong>{file.name}</strong><span>{(file.size / 1024).toFixed(0)} KB</span>{preview && <audio controls src={preview} />}</div>}<p className="muted">Audio is normalized in your browser to canonical mono 16 kHz PCM WAV before secure owner-scoped storage.</p></div>

    <div className="card analysis-action-card"><div className="eyebrow">02 · Interpret</div><h2>Ready when you are.</h2><p className="muted">{file ? `${file.name} is prepared for analysis.` : 'Record or upload a signal above to continue.'}</p><button className="primary" onClick={analyze} disabled={!file || !petId || busy}>{busy ? 'Analyzing…' : t.analyze + ' →'}</button><p className="microcopy">Your audio is processed for this analysis and stored under your account.</p></div>

    {result && <div className="result-card"><div className="result-topline"><div className="eyebrow">03 · PET AI interpretation</div><span className={`result-status ${result.safety_flag ? 'result-status-warning' : ''}`}>{result.safety_flag ? 'Welfare attention' : 'Evidence-based hypothesis'}</span></div><div className="row-between"><div><span className="result-label">{t.mostLikely}</span><h2>{result.likely_intent}</h2></div><div className="confidence">{Math.round(result.confidence * 100)}%<small>{t.confidence}</small></div></div><p className="emotion">{result.emotional_state}</p><div className="interpretation-summary"><div><span>Primary hypothesis</span><strong>{result.likely_intent}</strong></div><div><span>Heuristic confidence</span><strong>{Math.round(result.confidence * 100)}%</strong></div><div><span>Species</span><strong>{result.species}</strong></div></div>{result.acoustic_features && <div className="evidence-panel"><div className="eyebrow">{t.evidence}</div><div className="evidence-grid"><div><small>{t.duration}</small><strong>{(result.acoustic_features.durationMs / 1000).toFixed(1)}s</strong></div><div><small>{t.energy}</small><strong>{Math.round(result.acoustic_features.rms * 100)}%</strong></div><div><small>{t.arousal}</small><strong>{result.acoustic_features.estimatedArousal}</strong></div><div><small>{t.quality}</small><strong>{result.acoustic_features.acousticQuality}</strong></div><div><small>{t.signal}</small><strong>{result.acoustic_features.voiced ? 'Detected' : 'Weak'}</strong></div></div><p className="microcopy">These are acoustic measurements, not proof of emotion or intent.</p></div>}<div className="result-grid"><div className="result-evidence-block"><strong>{t.signals}</strong><p>{result.signals.join(' · ') || 'Insufficient signal evidence'}</p></div><div className="result-evidence-block"><strong>{t.alternatives}</strong><p>{result.alternative_interpretations.join(' · ') || 'None'}</p></div><div className="result-evidence-block"><strong>{t.context}</strong><p>{result.context_used.join(' · ') || 'No context supplied'}</p></div></div>{result.safety_flag && <div className="safety"><strong>{t.safety}</strong><p>Persistent, unusual, or severe distress should be assessed by a qualified veterinarian.</p></div>}<p className="notice">AI-assisted interpretation only. This confidence is a rule-based heuristic, not a scientifically calibrated probability. Acoustic evidence alone cannot establish intent. Not literal pet-language translation and not a veterinary diagnosis. Model: {result.model_version}</p><div className="feedback"><div><strong>Teach PET AI</strong><span>Was this interpretation useful for your pet?</span></div><button onClick={() => sendFeedback('correct')}>{t.correct}</button><button onClick={() => sendFeedback('partly')}>{t.partly}</button><button onClick={() => sendFeedback('incorrect')}>{t.incorrect}</button></div>{feedback && <p className="muted">{feedback}</p>}</div>}
  </div>
}
